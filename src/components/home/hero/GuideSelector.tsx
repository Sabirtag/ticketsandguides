...
const GuideSelector = ({
  ...
  return <div>
      <Popover open={showGuideOptions} onOpenChange={setShowGuideOptions}>
        ...
        <PopoverContent className="w-auto sm:w-80 border-0 bg-white rounded-md px-0 py-[11px]">
          <div className="space-y-2 p-2">
            {["choose_for_me", "choose_own", "no_guide"].map(choice => <Button key={choice} variant="ghost" onClick={() => handleGuideSelection(choice)} className="w-full justify-start text-xs rounded-sm transition-none bg-orange-50 text-black font-normal sm:text-base text-center">
