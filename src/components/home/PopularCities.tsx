...
const PopularCities: React.FC<PopularCitiesProps> = ({
  userLocation
}) => {
  ...
  useEffect(() => {
    if (userLocation && userLocation.latitude && userLocation.longitude) {
      const citiesWithDistance = allCities.map(city => ({
        ...city,
        distance: city.latitude && city.longitude ? calculateDistance(userLocation.latitude, userLocation.longitude, city.latitude, city.longitude) : Infinity
      }));
  ...
  return <section className="py-8 md:py-12 bg-[#f8f9fa]">
      <div className="container px-4 md:px-6">
        ...
        <div>
          <CityTabs cities={sortedCities} activeTab={activeTab} onTabChange={setActiveTab} onCityClick={handleCityClick} />
        </div>
      </div>
    </section>;
};
