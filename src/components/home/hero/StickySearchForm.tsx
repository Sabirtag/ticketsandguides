...
const StickySearchForm: React.FC<StickySearchFormProps> = ({
  ...
  return <div className={cn("fixed left-0 right-0 z-40 bg-white shadow-md transform transition-all duration-300", isVisible ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0 pointer-events-none")} style={{
        ...
        <form onSubmit={handleBook} className="flex items-center gap-2 max-w-4xl w-full">
          <div className="relative flex-grow max-w-xs">
            <div className="flex items-center bg-white border border-border rounded-md overflow-hidden">
              ...
              <Input placeholder="Where to" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onFocus={() => setShowSuggestions(true)} className="border-0 bg-transparent p-0 h-8 focus-visible:ring-0 pl-1" />
            </div>
            {showSuggestions && <div className="absolute top-full left-0 right-0 z-50">
          ...
          <div className="w-24">
            <VisitorSelector visitors={visitors} setVisitors={setVisitors} showVisitors={showVisitors} setShowVisitors={setShowVisitors} isCompact={true} />
          </div>
          
          <div className="w-24">
            <GuideSelector guideChoice={guideChoice} showGuideOptions={showGuideOptions} setShowGuideOptions={setShowGuideOptions} setGuideChoice={setGuideChoice} setShowGuidePreferences={setShowGuidePreferences} isCompact={true} />
          </div>
          
          <Button type="submit" size="sm" variant="cta" className="bg-[rgba(100,73,37,255)] text-white hover:bg-[rgba(100,73,37,0.9)]">
