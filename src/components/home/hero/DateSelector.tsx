  ...
  return <div>
      <Popover>
        ...
        <PopoverContent className="w-auto p-0 bg-[#FDE1D3] border-0" align="start">
          <Calendar mode="single" selected={date} onSelect={setDate} initialFocus disabled={date => date < new Date(new Date().setHours(0, 0, 0, 0))} className="pointer-events-auto text-gray-900 bg-amber-50 rounded-md" />
        </PopoverContent>
      </Popover>
    </div>;
};
