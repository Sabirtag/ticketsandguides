  ...
  return <div>
      ...
      <Popover open={showVisitors} onOpenChange={setShowVisitors}>
        <PopoverTrigger asChild>
          <Button variant="outline" className="w-full justify-start text-left font-normal bg-white/90 text-gray-900 border-0 h-9 sm:h-10 text-xs sm:text-sm">
            {getTotalVisitors()}
          </Button>
        </PopoverTrigger>
          ...
          <div className="space-y-3 sm:space-y-4">
            {visitors.map(category => <div key={category.type} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 sm:gap-2">
                  <Button variant="outline" size="icon" className="h-7 w-7 sm:h-8 sm:w-8 bg-white text-[rgba(100,73,37,255)] border-[rgba(100,73,37,255)] hover:bg-[rgba(100,73,37,0.1)]" onClick={() => handleVisitorCountChange(category.type, false)}>
                    <Minus className="h-3 w-3 sm:h-4 sm:w-4" />
                  </Button>
                  <span className="w-6 sm:w-8 text-center text-xs sm:text-sm">{category.count}</span>
                  <Button variant="outline" size="icon" className="h-7 w-7 sm:h-8 sm:w-8 bg-white text-[rgba(100,73,37,255)] border-[rgba(100,73,37,255)] hover:bg-[rgba(100,73,37,0.1)]" onClick={() => handleVisitorCountChange(category.type, true)}>
