      ...
      <Popover open={showGuideOptions} onOpenChange={setShowGuideOptions}>
        ...
        isCompact ? "h-8 px-2 py-1" : "h-9 sm:h-10", "text-xs sm:text-sm")}>
            ...
            <span className="truncate text-left text-slate-950 text-sm mx-0 my-0 py-0 px-0">
              {isCompact ? guideChoice ? guideChoice === "choose_for_me" ? "Guide chosen" : guideChoice === "choose_own" ? "Own guide" : "No guide" : "With" : guideChoice === "choose_for_me" ? "Guide chosen for you" : guideChoice === "choose_own" ? "Choose own guide" : guideChoice === "no_guide" ? "No guide" : "Get a Guide?"}
            </span>
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-auto sm:w-80 border-0 bg-neutral-50">
          <div className="space-y-2 p-2">
            {["choose_for_me", "choose_own", "no_guide"].map(choice => <Button key={choice} variant="ghost" onClick={() => handleGuideSelection(choice)} className="w-full justify-start text-xs text-center transition-none bg-orange-50 text-black font-normal sm:text-base px-px py-[21px] rounded-md add border bg black ">
