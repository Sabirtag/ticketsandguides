...
const VisitorSelector = ({
  ...
  return <div className="bg-white rounded-lg px-[36px]">
      <Popover open={showVisitors} onOpenChange={setShowVisitors}>
        <PopoverTrigger asChild>
          <Button variant="outline" onKeyDown={handleKeyDown} className="bg-white text-gray-500 text-sm rounded-lg text-center px-[36px]">
          ...
          <div className="space-y-3 sm:space-y-4">
            {visitors.map(category => <div key={category.type} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 sm:gap-2">
                  <Button variant="outline" size="icon" className="h-7 w-7 sm:h-8 sm:w-8 bg-white text-[rgba(100,73,37,255)] border-[rgba(100,73,37,255)] hover:bg-white" onClick={() => handleVisitorCountChange(category.type, false)}>
                    <Minus className="h-3 w-3 sm:h-4 sm:w-4" />
                  </Button>
                  <span className="w-6 sm:w-8 text-center text-xs sm:text-sm text-gray-900">{category.count}</span>
                  <Button variant="outline" size="icon" className="h-7 w-7 sm:h-8 sm:w-8 bg-white text-[rgba(100,73,37,255)] border-[rgba(100,73,37,255)] hover:bg-white" onClick={() => handleVisitorCountChange(category.type, true)}>
