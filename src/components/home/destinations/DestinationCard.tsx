      ...
      <div className="relative rounded-xl overflow-hidden cursor-pointer hover:shadow-md transition-all h-full">
        <div className="relative overflow-hidden aspect-[3/4]">
          ...
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent pt-12 pb-4 px-4">
            <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-white">{destination.name}</h3>
            ...
            <div className="flex items-center text-white/90 text-xs sm:text-sm mt-3">
          ...
          {destination.id === 5 && (
            <div className="absolute top-0 left-0 w-full h-full bg-blue-800/90 text-white p-4 flex flex-col">
              ...
              <p className="text-xs sm:text-sm">Open from Monday to Friday</p>
              <p className="text-xs sm:text-sm mb-4">Closes at 10PM</p>
              
              <div className="mt-auto">
                <p className="text-xs sm:text-sm font-medium">Tickets:</p>
                <p className="text-xs sm:text-sm">From ₹{parseInt(destination.price?.split(' ')[0].replace(/[^\d]/g, '') || '0')} per person</p>
                
                <div className="flex items-center justify-between mt-3">
                  <button className="text-xs sm:text-sm font-medium underline flex items-center mobile-tap-target">
